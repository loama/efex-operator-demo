              ...
              <Card style={styles.flushCard}>
                {data.accounts.map((account, index) => (
                  <Pressable accessibilityLabel={`Abrir cuenta ${account.name}`} accessibilityRole="button" key={account.id} onPress={() => router.push("/accounts")} style={[styles.accountRow, index > 0 && styles.rowBorder]}>
                    ...
                    <View style={styles.rowAmount}><Text numberOfLines={1} style={styles.rowValue}>{compactMoney(account.balance, account.currency)}</Text><Text numberOfLines={1} style={styles.rowMeta}>Disponible {compactMoney(account.available, account.currency)}</Text></View>
              ...
              <Card style={styles.flushCard}>
                {data.activity.slice(0, 4).map((item, index) => (
                  <View key={item.id} style={[styles.activityRow, index > 0 && styles.rowBorder]}>
                    ...
                    <View style={styles.rowAmount}><Text numberOfLines={1} style={[styles.rowValue, item.direction === "in" && styles.positive]}>{item.direction === "in" ? "+" : ""}{compactMoney(item.amount, item.currency)}</Text><Text style={styles.rowMeta}>{item.status === "approved" ? "Aprobado" : "En proceso"}</Text></View>
                  </View>
                ))}
              </Card>
            </View>
...
const styles = StyleSheet.create({
  ...
  accountRow: { alignItems: "center", flexDirection: "row", gap: 12, minHeight: 76, paddingHorizontal: 16 }, activityRow: { alignItems: "center", flexDirection: "row", gap: 12, minHeight: 72, paddingHorizontal: 16 }, rowBorder: { borderTopColor: colors.line, borderTopWidth: 1 }, currencyMark: { alignItems: "center", backgroundColor: colors.yellow, borderRadius: 18, flexShrink: 0, height: 36, justifyContent: "center", width: 36 }, currencyMarkText: { color: colors.ink, fontFamily: fonts.headingBold, fontSize: 14 }, rowCopy: { flex: 1, gap: 4, minWidth: 0 }, rowTitle: { color: colors.ink, fontFamily: fonts.bodySemiBold, fontSize: 13 }, rowMeta: { color: colors.muted, fontFamily: fonts.body, fontSize: 10 }, rowAmount: { alignItems: "flex-end", flexShrink: 1, gap: 4, minWidth: 0 }, rowValue: { color: colors.ink, fontFamily: fonts.bodySemiBold, fontSize: 12 }, positive: { color: colors.green }, activityIn: { alignItems: "center", backgroundColor: colors.greenSoft, borderRadius: 18, flexShrink: 0, height: 36, justifyContent: "center", width: 36 }, activityOut: { alignItems: "center", backgroundColor: colors.canvas, borderRadius: 18, flexShrink: 0, height: 36, justifyContent: "center", width: 36 },
