...
export default function PaymentsScreen() {
  ...
  return <AppShell title="Pagos" subtitle="Crea y revisa transferencias simuladas." action={<Button icon="add" label="Nuevo pago" onPress={() => router.push("/payments/new")} />}>
    ...
    {items.length ? <Card style={styles.list}>{items.map((payment, index) => <View key={payment.id} style={[styles.row, index > 0 && styles.border]}>
      ...
      <View style={styles.copy}><Text numberOfLines={1} style={styles.name}>{payment.beneficiaryName}</Text><Text numberOfLines={1} style={styles.meta}>{payment.reference} · {new Date(payment.createdAt).toLocaleDateString("es-MX")}</Text></View>
      <View style={styles.amount}><Text numberOfLines={1} style={styles.value}>{compactMoney(payment.sourceAmount, payment.sourceCurrency)}</Text><Text numberOfLines={1} style={styles.meta}>{compactMoney(payment.destinationAmount, payment.destinationCurrency)}</Text></View>
...
const styles = StyleSheet.create({
  list: { padding: 0 }, row: { alignItems: "center", flexDirection: "row", gap: 10, minHeight: 78, paddingHorizontal: 14 }, border: { borderTopColor: colors.line, borderTopWidth: 1 }, icon: { alignItems: "center", backgroundColor: colors.yellow, borderRadius: 18, flexShrink: 0, height: 36, justifyContent: "center", width: 36 }, copy: { flex: 1, gap: 4, minWidth: 0 }, name: { color: colors.ink, fontFamily: fonts.bodySemiBold, fontSize: 13 }, meta: { color: colors.muted, fontFamily: fonts.body, fontSize: 10 }, amount: { alignItems: "flex-end", flexShrink: 1, gap: 4, minWidth: 0 }, value: { color: colors.ink, fontFamily: fonts.bodySemiBold, fontSize: 12 },
});
