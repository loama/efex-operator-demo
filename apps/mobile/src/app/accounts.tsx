      ...
      {resource.data?.accounts.length ? <>
        <View style={[styles.grid, !desktop && styles.gridMobile]}>
          {resource.data.accounts.map((account) => <Card key={account.id} style={styles.accountCard}>
            ...
            <Text adjustsFontSizeToFit minimumFontScale={0.58} numberOfLines={1} style={styles.balance}>{money(account.balance, account.currency)}</Text>
            <View style={styles.availableRow}><Text style={styles.availableLabel}>Disponible</Text><Text adjustsFontSizeToFit minimumFontScale={0.7} numberOfLines={1} style={styles.availableValue}>{money(account.available, account.currency)}</Text></View>
          </Card>)}
        </View>
