...
import { Avatar, Button, Card, Field, LoadingState, Pill } from "../../components/ui";
...
export default function NewPaymentScreen() {
  ...
  const [amount, setAmount] = useState("25000"); const [reference, setReference] = useState("Invoice 1088"); const [quote, setQuote] = useState<Quote>(); const [payment, setPayment] = useState<Payment>();
  ...
  useEffect(() => { if (step === "done") Animated.spring(scale, { toValue: 1, friction: 5, tension: 80, useNativeDriver: true }).start(); }, [scale, step]);

  async function getQuote() {
    const value = Number(amount);
    if (!Number.isFinite(value) || value <= 0) return setError("Ingresa un monto válido");
    setLoading(true); setError(undefined);
    try { setQuote(await api.quote(value)); setStep("review"); } catch (reason) { setError(reason instanceof Error ? reason.message : "No fue posible cotizar"); } finally { setLoading(false); }
  }
  async function send() {
    if (!selected || !quote) return;
    setLoading(true); setError(undefined);
    try {
      const draft = await api.createPayment({ beneficiaryId: selected.id, sourceCurrency: "USD", destinationCurrency: "MXN", sourceAmount: quote.sourceAmount, reference });
  ...
  return <AppShell title={title} subtitle="La transferencia se procesa únicamente dentro del entorno demo.">
    ...
    {step === "beneficiary" && !loading ? <Card style={styles.panel}><Text style={styles.panelTitle}>Elige un beneficiario</Text>{beneficiaries.map((item) => <Pressable key={item.id} onPress={() => { setSelected(item); setStep("amount"); }} style={styles.beneficiary}><Avatar label={initials(item.name)} /><View style={styles.flex}><Text style={styles.name}>{item.name}</Text><Text style={styles.meta}>{item.bank} · {item.accountNumber}</Text></View><Ionicons color={colors.muted} name="chevron-forward" size={18} /></Pressable>)}</Card> : null}
    {step === "amount" && selected ? <Card style={styles.panel}>
      <View style={styles.selected}><Avatar label={initials(selected.name)} /><View style={styles.flex}><Text style={styles.name}>Para {selected.name}</Text><Text style={styles.meta}>{selected.bank} · MXN</Text></View><Button label="Cambiar" onPress={() => setStep("beneficiary")} variant="quiet" /></View>
    ...
    {step === "review" && selected && quote ? <Card style={styles.panel}>
      <View style={styles.reviewTop}><View><Text style={styles.reviewLabel}>Envías</Text><Text style={styles.reviewAmount}>{money(quote.sourceAmount, "USD")}</Text></View><Ionicons color={colors.ink} name="arrow-forward" size={24} /><View style={styles.reviewRight}><Text style={styles.reviewLabel}>Recibe</Text><Text style={styles.reviewAmount}>{money(quote.destinationAmount, "MXN")}</Text></View></View>
      <View style={styles.summary}><Summary label="Beneficiario" value={selected.name} /><Summary label="Tipo de cambio" value={`1 USD = ${quote.rate.toFixed(2)} MXN`} /><Summary label="Comisión" value={money(quote.fee, "USD")} /><Summary label="Concepto" value={reference} /></View>
      ...
      {error ? <Text style={styles.error}>{error}</Text> : null}<View style={styles.actions}><Button label="Atrás" onPress={() => setStep("amount")} variant="secondary" /><Button label="Confirmar pago demo" loading={loading} onPress={() => void send()} /></View>
    </Card> : null}
