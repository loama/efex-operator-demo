      ...
      <View style={styles.chatColumn}>
        ...
        <Card style={styles.chat}>
          <View style={styles.messages}>{messages.map((message) => <View key={message.id} style={[styles.bubble, message.role === "user" ? styles.userBubble : styles.assistantBubble]}>
            ...
            {message.response?.action ? <Pressable accessibilityLabel={message.response.action.label} accessibilityRole="button" onBlur={() => setFocusedControl(undefined)} onFocus={() => setFocusedControl(`${message.id}:action`)} onPress={() => router.push(message.response!.action!.route as Href)} style={[styles.actionCard, focusedControl === `${message.id}:action` && styles.controlFocused]}><Text style={styles.actionLabel}>{message.response.action.label}</Text><Ionicons color={colors.ink} name="arrow-forward" size={17} /></Pressable> : null}
